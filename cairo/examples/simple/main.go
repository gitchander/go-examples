...
func (e *Example) Execute() cairo.Status {

	fileName := filepath.Join(e.Dir, e.FileName)

	surface := cairo.NewSurface(cairo.FORMAT_ARGB32, e.Size.Width, e.Size.Height)
...
func ExampleClipImage(c *cairo.Canvas) {
	...
	image, status = cairo.NewSurfaceFromPNG(textureDefiance1)
	if status != cairo.STATUS_SUCCESS {
...
func ExampleGradient(c *cairo.Canvas) {
	...
	// draw background
	{
		p := cairo.NewPatternLinear(0.0, 0.0, 0.0, 256.0)
...
func ExampleImage(c *cairo.Canvas) {
	...
	image, status := cairo.NewSurfaceFromPNG(textureDefiance2)
	if status != cairo.STATUS_SUCCESS {
		fmt.Println(cairo.StatusToString(status))
		return
	}
...
func ExampleImagePattern(c *cairo.Canvas) {
	...
	image, status = cairo.NewSurfaceFromPNG(textureCircleBlue)
	if status != cairo.STATUS_SUCCESS {
		fmt.Println(cairo.StatusToString(status))
		return
	}
	...
	pattern = cairo.NewPatternForSurface(image)
...
func main() {
	...
	for _, e := range es {
		if status := e.Execute(); status != cairo.STATUS_SUCCESS {
			fmt.Println(cairo.StatusToString(status))
			return
		}
	}
